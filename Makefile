...
R_STUDIO_PORT=8787                # modify this number so that there's no conflict with other users
# END OF CONFIG  =============================================================================================================
...
docker-run:
	NV_GPU=$(AVAILABLE_GPUS) docker run -it -p $(VSCODE_PORT):8443 -p $(LOCAL_JUPYTER_PORT):8888 \
    -p $(LOCAL_TENSORBOARD_PORT):6006 -p $(R_STUDIO_PORT):8787 -v $(shell pwd):/notebooks --name $(CONTAINER) $(IMAGE)
